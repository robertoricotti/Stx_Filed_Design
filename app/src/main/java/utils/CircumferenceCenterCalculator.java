...
public class CircumferenceCenterCalculator {
    ...
    // Metodo per calcolare il centro della circonferenza dati tre punti
    public static double[] findCircumferenceCenter(double x1, double y1, double z1,
                                                   double x2, double y2, double z2,
                                                   double x3, double y3, double z3) {
        // Calcola i punti medi tra le coppie di punti
        double midPointX1 = (x1 + x2) / 2;
        double midPointY1 = (y1 + y2) / 2;
        double midPointZ1 = (z1 + z2) / 2;

        double midPointX2 = (x2 + x3) / 2;
        double midPointY2 = (y2 + y3) / 2;
        double midPointZ2 = (z2 + z3) / 2;
        ...
        // Calcola la coordinata z del centro della circonferenza usando il punto 1
        double centerZ = ((x1 - centerX) * (x1 - centerX) + (y1 - centerY) * (y1 - centerY) + z1 * z1 - x1 * x1 - y1 * y1) / 2.0;


        // Restituisci le coordinate x, y, z,raggio e angolo del centro della circonferenza come un array
        return new double[]{centerX, centerY, centerZ,calculateRadiusAndAngle(centerX,centerY,centerZ,x3,y3,z3)[0],calculateRadiusAndAngle(centerX,centerY,centerZ,x3,y3,z3)[1]};
    }

     static double[] calculateRadiusAndAngle(double xC, double yC, double zC,
                                                   double xP, double yP, double zP) {
        // Calcola la lunghezza del raggio
        double radius = Math.sqrt(Math.pow(xP - xC, 2) + Math.pow(yP - yC, 2) + Math.pow(zP - zC, 2));
