...
public class PLC_DataTypes_LittleEndian {
    ...
    public static int byte_to_U16(byte[] data) {
        if (data == null) {
            ...
            if (data.length != 2) {
                throw new IllegalArgumentException("Il byte array deve avere esattamente 2 elementi.");
            }
    ...
    public static int byte_to_S32(byte[] bytes) {
        if (bytes.length != 4) {
            throw new IllegalArgumentException("Il byte array deve avere esattamente 4 elementi.");
        }
    ...
    public static long byte_to_S64_le(byte[] bytes) {
        ...
        if (bytes.length != 8) {
            throw new IllegalArgumentException("L'array deve avere 8 byte.");
        }
        long value = 0;
        for (int i = 0; i < 8; i++) {
    ...
    */
    ...
    public static byte[] U16_to_bytes(int value) {
        if (value < 0 || value > 65535) {
            throw new IllegalArgumentException("Il valore deve essere compreso tra 0 e 65535.");
        }
        byte[] bytes = new byte[2];
        bytes[0] = (byte) (value & 0xff);
        bytes[1] = (byte) ((value >> 8) & 0xff);
    ...
    public static byte[] U32_to_bytes(long value) {
        if (value < 0 || value > 4294967295L) {
            throw new IllegalArgumentException("Il valore deve essere compreso tra 0 e 4294967295.");
        }
        byte[] bytes = new byte[4];
        bytes[0] = (byte) (value & 0xff);
        bytes[1] = (byte) ((value >> 8) & 0xff);
